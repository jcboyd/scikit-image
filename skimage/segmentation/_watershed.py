...
def _validate_inputs_limited(limits, markers, mask, connectivity):
    """Ensure that all inputs to watershed have matching shapes and types.
    ...
    ValueError
        If the shapes of the given arrays don't match.
    """
    n_pixels = markers.size
    if mask is None:
        # Use a complete `True` mask if none is provided
        mask = np.ones(markers.shape, bool)
    else:
        mask = np.asanyarray(mask, dtype=bool)
        n_pixels = np.sum(mask)
        if mask.shape != markers.shape:
            message = ("`mask` (shape {}) must have same shape as "
                       "`markers` (shape {})".format(markers.shape, markers.shape))
            raise ValueError(message)

    return (limits.astype(np.float64),
            markers.astype(np.int32),
            mask.astype(np.int8))


def watershed_limited(markers, limits, mask=None, 
                      connectivity=1, offset=None, 
    ...
    separate overlapping spheres.
    """
    if markers.shape != limits.shape:
        message = ("`markers` (shape {}) must have same shape as "
                   "`limits` (shape {})".format(markers.shape, limits.shape))
        raise ValueError(message)    

    limits, markers, mask = _validate_inputs_limited(limits, markers, mask, connectivity)
    connectivity, offset = _validate_connectivity(markers.ndim, connectivity,
                                              offset)

    # pad the image, markers, and mask so that we can use the mask to
    # keep from running off the edges
    pad_width = [(p, p) for p in offset]
    limits = np.pad(limits, pad_width, mode='constant')
    output = np.pad(markers, pad_width, mode='constant')

    flat_neighborhood = _offsets_to_raveled_neighbors(
        markers.shape, connectivity, center=offset)
    marker_locations = np.flatnonzero(output)
    image_strides = np.array(output.strides, dtype=np.intp) // output.itemsize
    
    _watershed_cy.watershed_limited_raveled(marker_locations,
                                            flat_neighborhood,
                                            limits.ravel(), 
                                            mask.ravel(), 
                                            image_strides,
