...
@cython.boundscheck(False)
@cython.wraparound(False)
def watershed_limited_raveled(cnp.intp_t[::1] marker_locations,
                              cnp.intp_t[::1] structure,
                              cnp.float64_t[::1] limits,
                              DTYPE_BOOL_t[::1] mask,
                              cnp.intp_t[::1] strides,
                              DTYPE_INT32_t[::1] output,
                              DTYPE_BOOL_t wsl):
    ...
    with nogil:
        ...
        while hp.items > 0:
            ...
            for i in range(nneighbors):
                ...
                new_elem.value = _euclid_dist(neighbor_index, elem.source, strides)
                if new_elem.value > limits[elem.source]:
                    # this branch includes basin boundaries, aka watershed lines
                    # neighbor is not in mask
                    continue
